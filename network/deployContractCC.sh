...
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"function":"InitContract","Args":["1023"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3
...
infoln "TEST1 : Invoking the chaincode"
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"function":"UpdateContract","Args":["1023","user1","user2", "11111","2222", "register-contract","1","hashahsahs"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3
...
infoln "TEST1 : Invoking the chaincode"
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"function":"UpdateState","Args":["1023","complete"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3
...
infoln "TEST2 : Query the chaincode"
set -x
peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function":"ContractState","Args":["1023"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt